 ...
 * 1.增加destroy接口对外调用
 * 2.增加是否强制重绘设置，reload
 * 3.增加关联table配置，关联table会随着主table一起改变宽度
 * */
...
(function($){ 	
	...
	*/
	...
	function getIeVersion() {
		if(!IE_VERSION){
			var re = /msie\s+(\d+)/i.exec(navigator.userAgent);
			if(re){
				IE_VERSION = re[1];
			}
		}
	...
	var setRelation = function(pt) {
        var t = pt.opt.relation;
        if (t) {
            var width = pt.outerWidth() - 1;
            t.width( width );
            t.css('min-width', width);
            t.css('marginLeft',pt.css('marginLeft'));
            t.css('marginRight',pt.css('marginRight'));
            var th = t.find(">thead>tr:first>th,>thead>tr:first>td"); //table headers are obtained
            if(!th.length) th = t.find(">tbody>tr:first>th,>tr:first>th,>tbody>tr:first>td, >tr:first>td");
            var pth = pt.find(">thead>tr:first>th,>thead>tr:first>td"); //table headers are obtained
            if(!pth.length) pth = pt.find(">tbody>tr:first>th,>tr:first>th,>tbody>tr:first>td, >tr:first>td")
            for (var i = 0; i < pth.length; i++) {
                var w = pth.eq(i).width();
                // IE9,10下横向滚动条处理
                if (getIeVersion() && getIeVersion() < 11 && i == pth.length - 1) {
                    th.eq(i).width(w -1);
                } else {
                    th.eq(i).width(w);
                }
	...
	var onGripMouseDown = function(e){
		...
		// 修改正 mid-width为100%时，不能拖小的问题
		$(t).css({'width': $(t).width(), 'min-width': $(t).width()});
		// 修改关联表格 X 滚动条CSS
		var dtApi = $.fn.dataTable.Api(t.opt.relation);
		var dtSettings = dtApi.settings()[0];
		if(dtSettings){
			$(dtSettings.nScrollBody).css('overflow-x', 'auto');
		}
	...
	// 定时更新 拖动控制, 表格隐藏后对不齐的问题
	var __time__ = setInterval(function(){
		for (var t in tables) {
			if (tables.hasOwnProperty(t)) {
				// 表格已经删除清理
				if($('#'+t).size() == 0){
				...
				if(tables[t].ln == 0){
					createGrips(tables[t])
				}
				syncGrips(tables[t]);
				// 解决IE9下当有两列过长内容时，点击某行的过长内容后，鼠标离开这行时，表格会错位的问题。
				if(navigator.appName == "Microsoft Internet Explorer" 
					&& navigator.appVersion .split(";")[1].replace(/[ ]/g,"")=="MSIE9.0") {
					setRelation(tables[t]);
				}
			}
		}
	}, 250);
    ...
    $.fn.extend({  
        colResizable: function(options) {           
            var defaults = {
                ...
                relation: null,                 //关联table
                reload: false,                  //是否强制重新绘制
            ...
            var resObj = this.each(function() {
                init( this, options);
            });
            resObj.destroy = function() {
                return destroy(this);
			};
